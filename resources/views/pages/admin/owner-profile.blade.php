...
@section('content')
    ...
    @elseif($errors->any())
    <admin-owner-profile v-bind:message="{{ json_encode(['status' => 'failed', 'body' => $errors->all()]) }}" v-bind:user-info="{{$userInfo}}" v-bind:user-name="{{ $userName }}" v-bind:old-input="{{ json_encode(old()) }}"></admin-owner-profile>
    @else
