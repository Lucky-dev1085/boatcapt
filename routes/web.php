...
Route::get('/faq', 'LanderController@faq');

Route::get('/terms', 'LanderController@terms');
Route::get('/cancellation', 'LanderController@cancellation');
Route::get('/privacy', 'LanderController@privacy');
...
Route::group(['middleware' => ['auth']], function () {
    ...
    Route::get('/profile', function () {
        if(Auth::user()->role == 1002)
        {
            return redirect('/owner-profile');
        }
        else if(Auth::user()->role == 1003)
        {
            return redirect('/captain-profile');
        }
    });
});
...
Route::group(['middleware' => ['owner'], 'namespace'=>'Owner'], function () {
    ...
    Route::get('/{captainId}/{tripId}/send-request-captain', 'BookCaptainController@requestSend');

    Route::get('/owner-request-captain', 'BookCaptainController@anyRequest');
    Route::post('/owner-request-captain', 'BookCaptainController@bookAnyCaptain');
    Route::get('{tripId}/request-any-confirm', 'BookCaptainController@bookAnyConfirm');
    Route::post('/request-any-captain', 'BookCaptainController@requestAnyCaptain');
    Route::get('{tripId}/send-request-any-captain', 'BookCaptainController@requestAnySend');
...
Route::group(['middleware' => ['captain'], 'namespace'=>'Captain'], function () {
    ...
    Route::get('/captain-resume-download', 'ProfileController@downloadResume');
});
...
Route::group(['middleware' => ['admin']], function () {
    ...
    Route::group(['namespace'=>'Admin'], function () {
        ...
        Route::get('/{captainId}/admin-resume-download', 'CaptainController@downloadResume');
    });
