...
class DashboardController extends Controller
{
    ...
    public function infoList(Request $request) 
    {
        ...
        if(isset($request->startDate))
            $cOwner->where('users.created_at', '>', date('Y-m-d', strtotime($request->startDate)));
        if(isset($request->endDate))
            $cOwner->where('users.created_at', '<', date('Y-m-d', strtotime('+1 day', strtotime($request->endDate))));
        $owner = $cOwner->count();
