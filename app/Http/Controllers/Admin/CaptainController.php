...
class CaptainController extends Controller
{
    ...
    public function updateProfile(Request $request) 
    {
        ...
        if($avatar)
        {
            $fileName = 'profile-'.$userId.'.'.$avatar->getClientOriginalExtension();
        ...
        if($resumeFile)
        {
            ...
            {
                return redirect()->back()
                            ->withErrors(['message' => 'There was a problem uploading the resume file.'])
                            ->withInput();
            }
        }
    ...
    public function downloadResume($captainId) {

        $captainInfo = $this->captainInfo->where('user_id', $captainId)->first();  

        $file= public_path(). "/uploads/resumeFiles/resume-".$captainId;
        if (file_exists($file)) {
