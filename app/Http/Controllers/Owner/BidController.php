...
class BidController extends Controller
{
    ...
    public function acceptBid(Request $request) 
    {
        ...
        {       
            ...
            if($request->merchant_type == 1)
            {
                ...
                try {
                    ...
                    return redirect()->back()
                                    ->withErrors(['message' => 'There was a problem payment.']);
                }
        ...
        try {
            ...
            $bidRequests = $this->bidRequest->where('trip_id', $trip_id)->get();
            foreach ($bidRequests as $key => $bidRequest) {
                $bidRequest->delete();
            }
            
            $bids = $this->bid->where('trip_id', $trip_id)->get();
            foreach ($bids as $key => $bid) {
                $bid->delete();
            }

            if ( App::environment() === 'local' )
                Auth::user()->notify(new BidAccept($bidInfo));  
    ...
    public function completePaymentWithPayPal(Request $request, $bidId) {        
        ...
        {
            return redirect()->back()
                        ->withErrors(['message' => 'There was a problem bid info.']);
        }
    ...
    {
        $bids = $this->bid->where('id', $bidId)->get();
        foreach ($bids as $key => $bid) {
            $bid->delete();
        }
