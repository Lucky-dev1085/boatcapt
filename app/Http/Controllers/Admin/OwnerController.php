...
class OwnerController extends Controller
{
    ...
    public function ownerList(Request $request) 
    {
        ...
        if(isset($request->startDate))
            $ownerTotal->where('created_at', '>', date('Y-m-d', strtotime($request->startDate)));
        if(isset($request->endDate))
            $ownerTotal->where('created_at', '<', date('Y-m-d', strtotime('+1 day', strtotime($request->endDate))));
        ...
        $owners = $this->user->select('users.*')
                                    ->selectSub('users.created_at', 'date')
        ...
        if(isset($request->startDate))
            $owners->where('users.created_at', '>', date('Y-m-d', strtotime($request->startDate)));
        if(isset($request->endDate))
            $owners->where('users.created_at', '<', date('Y-m-d', strtotime('+1 day', strtotime($request->endDate))));
